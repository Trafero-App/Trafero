...
async def direct_nearby_routes_to_2_point(
    long, lat, radius, long2, lat2, radius2, mapbox_token
):
    """
    ...
    for route_id in routes_near_A:
        if route_id not in routes_near_B:
            continue
    ...
    for id_A in routes_A:
        ...
        for id_B in routes_B:
            ...
            for (
                ...
                route2_id,
                route2_intersection,
            ) in intersection_data:
                if route1_id != id_A or route2_id != id_B:
                    continue
                if (
                    pickup_index > route1_intersection
                    or destination_index < route2_intersection
                ):
                    continue
                chain = Chain(
                    route1_id,
...
async def filter_duplicate_chains(chains):
    ...
    for i, chain1 in enumerate(chains):
        for chain2 in chains[i + 1 :]:
            if (
                chain1.route1_id == chain2.route1_id
                and chain1.route2_id == chain2.route2_id
            ):
                length_1 = (
                    chain2.route1_intersection
                    - chain1.pickup_index
                    + chain1.dest_index
                    - chain1.route2_intersection
                )
                length_2 = (
                    chain2.route1_intersection
                    - chain2.pickup_index
                    + chain2.dest_index
                    - chain2.route2_intersection
                )
...
async def chained_routes(intersections, nearby_A, nearby_B, mapbox_token):
    ...
    for chain in filtered_chains:
        ...
        eta1 = await get_time_estimation(
            route1_id, chain.pickup_index, chain.route1_intersection, mapbox_token
        )
        sliced_1 = route1_data["line1"]["features"][0]["geometry"]["coordinates"][
            chain.pickup_index : chain.route1_intersection + 1
        ]
        route1_data["eta1"] = eta1

        route2_id = chain.route2_id
        route2_data = await get_route_details(route2_id, 2)
        eta2 = await get_time_estimation(
            route2_id, chain.route2_intersection, chain.dest_index, mapbox_token
        )
        sliced_2 = route2_data["line2"]["features"][0]["geometry"]["coordinates"][
            chain.route2_intersection : chain.dest_index + 1
        ]
                ...
                "features": [
                    {
                        "type": "Feature",
                        "properties": {"order": 1},
                        "geometry": {"coordinates": sliced_1, "type": "LineString"},
                    },
                    {
                        "type": "Feature",
                        "properties": {"order": 2},
                        "geometry": {"coordinates": sliced_2, "type": "LineString"},
                    },
                ],
            },
        }
...
async def all_nearby_routes_2_points(
    long, lat, radius, long2, lat2, radius2, mapbox_token
):
    """
    ...
    close_direct_routes = await direct_nearby_routes_to_2_point(
        long, lat, radius, long2, lat2, radius2, mapbox_token
    )
    close_chained_routes = await chained_routes(
        intersections, routes_near_A, routes_near_B, mapbox_token
    )
