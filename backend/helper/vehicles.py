...
async def all_vehicles_info():
    ...
    for vehicle in vehicle_info:
        ...
        if vehicle["longitude"] is None:
            continue
        if vehicle["status"] != "unknown":
            vehicle_coords = project_point_on_route(
        ...
        features.append(
            {
                "type": "Feature",
                "properties": {"status": vehicle["status"], "id": vehicle["id"]},
                "geometry": {"coordinates": vehicle_coords, "type": "Point"},
            }
        )
...
async def get_route_vehicles_arrival_status(pick_up, vehicles, route_id, token):
    ...
    for vehicle in vehicles:
        vehicle_index = project_point_on_route(
            (vehicle["longitude"], vehicle["latitude"]), route_id
        )[0]
        ...
        if pickup_index < vehicle_index:
            ...
            waypoints_lists[vehicle["id"]] = trim_waypoints(
                waypoints, route_id, start_index=vehicle_index, end_index=pickup_index
            )
            available.append(vehicle)

    all_vehicles_etas = await get_all_etas(
        list(waypoints_lists.values()), token, "driving"
    )

    for i, vehicle_id in enumerate(waypoints_lists.keys()):
        if "Van" in db.routes[route_id]["details"]["route_name"]:
            all_vehicles_etas[i] *= 1.2
            all_vehicles_etas[i] = round(all_vehicles_etas[i] / 60)
        else:
