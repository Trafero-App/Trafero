...
class Account_Info(BaseModel):
    ...
    status: (
        Literal["active", "unknown", "inactive", "unavailable", "waiting"] | None
    ) = None
    ...
    def validate_account_info(cls, values):
        ...
        if not is_valid_name(values.first_name) or not is_valid_name(values.last_name):
            raise ValueError(
                "Name must consist of only alphabetic characters, spaces, dashes, and apostrophes"
            )
        ...
        if values.account_type == "driver":
            ...
            if values.cur_route_id is None:
                values.cur_route_id = values.routes[0]
            if values.status is None:
                values.status = "inactive"
            if values.vehicle_type is None:
                raise ValueError("Please specify the vehicle's type")
            if values.brand is None:
                raise ValueError("Please specify the vehicle's brand")
            if values.model is None:
                raise ValueError("Please specify the vehicle's model")
            if values.license_plate is None:
                raise ValueError("Please specify your license plate")
            if not is_valid_license_plate(values.license_plate):
                raise ValueError("Please provide a valid license plate")
            if values.vehicle_color is None:
                raise ValueError("Please specify the vehicle's color")
            if values.routes in (None, []):
                raise ValueError("Please provide a non-empty list of routes")
        return values
    ...
    def validate_review(cls, values):
        if values.get("reaction") == False:
            if values.get("complaints") is None:
                raise ValueError(
                    "Please include a list of complaints for negative reviews."
                )
